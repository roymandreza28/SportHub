...
export const input =
  'w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 shadow-xs transition-all duration-150 focus:border-teal-500 focus:outline-none focus:ring-4 focus:ring-teal-100'
...
const buttonBase =
  'inline-flex items-center justify-center gap-1.5 rounded-xl px-4 py-2.5 text-sm font-semibold transition-all duration-150 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50 disabled:active:scale-100'
...
export const buttonPrimary = `${buttonBase} bg-teal-600 text-pure-white shadow-sm shadow-teal-600/20 hover:bg-teal-700 hover:shadow-md hover:shadow-teal-600/25`
...
export const chip = (active: boolean) =>
  `rounded-full px-3.5 py-1.5 text-xs font-medium transition-all duration-150 ${
    active ? 'bg-teal-600 text-pure-white shadow-sm shadow-teal-600/20' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
  }`

export const card = 'flex flex-col gap-3 rounded-xl border border-slate-100 bg-slate-50/60 p-4'
