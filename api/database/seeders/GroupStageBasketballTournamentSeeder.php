...
// A main-organizer Basketball 5v5 tournament in GROUP_STAGE format — same
// full-detail treatment as FullDetailBasketballTournamentSeeder, exercising
// this format instead. 4 teams (the minimum requested) collapses to exactly
// ONE group of 4 (see BracketService::generateGroupStage()'s
// ceil(count/4) group count) — a full round robin within that group, then
// its top 2 finishers advance into a single knockout match, which IS the
// tournament final. Every group match is completed with a full box score;
// BracketService::maybeStartGroupKnockout() auto-generates and auto-seeds
// that final the instant the 6th group match completes, which is left
// scheduled — "already at the final" without the tournament actually being
// over, same framing as every sibling seeder in this file group.
class GroupStageBasketballTournamentSeeder extends Seeder
{
    private const COACH_EMAILS = ['coach1@sporthub.test', 'coach2@sporthub.test', 'coach3@sporthub.test', 'coach4@sporthub.test'];

    private const TEAM_NAMES = ['Mahabang Parang Mustangs', 'Malakaban Marauders', 'Mambog Monarchs', 'Pila-Pila Panthers'];
    ...
    public function run(): void
    {
        ...
        $tournament = Tournament::create([
            ...
            'name' => 'Binangonan Group Stage Basketball Cup',
            'sport_id' => $sport->id,
        ...
        // The single group's 6 round-robin matches — completing the last
        // one triggers BracketService::maybeStartGroupKnockout(), which
        // auto-generates and auto-seeds the knockout final from the
        ...
        // The knockout final — auto-generated above, both qualifiers
        // already seeded into it. Left scheduled.
        GameMatch::where('bracket_id', $bracket->id)->whereNull('group_number')->first()
            ?->update(['scheduled_at' => now()->addMinutes(10), 'court_id' => $court->id]);

        $this->command?->info("Seeded '{$tournament->name}' (id {$tournament->id}) — group stage, 4 teams (1 group), {$players->count()} players, group round completed with full box scores/stat sheets/match logs, knockout final pending.");
    }
