...
// A main-organizer Basketball 5v5 tournament in DOUBLE_ELIMINATION format —
// same full-detail box score/stat sheet/match log treatment as
// FullDetailBasketballTournamentSeeder (single_elimination), just exercising
// a different bracket shape. 4 teams (the minimum that produces a real
// winners+losers bracket — see BracketService::generateDoubleElimination(),
// which skips the losers bracket entirely below 2 winners-bracket rounds),
// with every match EXCEPT the grand final played out: both winners-bracket
// rounds, both losers-bracket rounds. The grand final (round 200,
// bracket_type 'final') is left scheduled with both finalists already
// seeded into it — "already at the final" without the tournament actually
// being over, same framing as every sibling seeder in this file group.
class DoubleEliminationBasketballTournamentSeeder extends Seeder
{
    private const COACH_EMAILS = ['coach1@sporthub.test', 'coach2@sporthub.test', 'coach3@sporthub.test', 'coach4@sporthub.test'];

    private const TEAM_NAMES = ['Ithan Islanders', 'Janosa Juggernauts', 'Kalinawan Kings', 'Kasile Comets'];
    ...
    public function run(): void
    {
        ...
        $tournament = Tournament::create([
            ...
            'name' => 'Binangonan Double-Elimination Basketball Cup',
            'sport_id' => $sport->id,
        ...
        // Winners-bracket round 1 (2 matches) — both participants already
        // seeded at generation, no byes with exactly 4 teams.
        $this->playRound($bracket, 'winners', 1, $bracketService);
        ...
        // both finalist slots.
        $this->playRound($bracket, 'losers', 102, $bracketService);
        ...
        $this->command?->info("Seeded '{$tournament->name}' (id {$tournament->id}) — double elimination, 4 teams, {$players->count()} players, winners+losers brackets completed with full box scores/stat sheets/match logs, grand final pending.");
    }

    private function playRound(\App\Models\Bracket $bracket, string $bracketType, int $round, BracketService $bracketService): void
    {
